...
- **볼린저 밴드폭 스퀴즈(`bb_squeeze`)**: 밴드 폭이 최근 대비 얼마나 좁은지(0~1). 방향성 없는 신호라 부호 있는 수익률이 아니라 `|향후 3일 수익률|`과 비교 — **corr -0.0824, 상위20%(스퀴즈 강함) vs 하위20% |수익률| 스프레드 -0.851%p**. 기존 지표 전부(≤0.03)보다 3배 이상 큰, n=17,605 기준으로도 확실히 유의한 상관관계지만, **"스퀴즈 후 큰 돌파"라는 트레이더 통념과 정반대 방향** — 밴드가 좁을수록(최근 변동성 낮음) 오히려 향후 변동폭도 작았음. 변동성 군집화(volatility clustering, 최근 변동성이 낮으면 당분간도 낮게 유지되는 잘 알려진 시장 현상)와 일치하는 해석 — "곧 터진다"가 아니라 "조용하면 계속 조용하다"는 쪽. **아직 미확인**: 이미 동적 손절/목표가에 쓰이는 ATR%(둘 다 최근 변동성을 재는 지표)와 사실상 같은 정보를 재는 것인지, 아니면 별개 정보인지 — 두 값의 상관관계를 확인해야 실질적 활용 여부(예: 예상 등락률 크기를 스퀴즈 상태에 따라 보수적으로 조정) 판단 가능. 표본 크기가 커서(n=17,605) 통계적 유의성 자체는 이 프로젝트가 경계해온 "작은 표본 유의성" 사례는 아님
...
- **결론**: ADX 필터/CMF/OBV 셋은 전부 기존 지표의 노이즈 수준(±0.03)을 못 벗어나 production 반영 근거 없음 — 셋 다 "오실레이터/거래량 계열의 파생 지표는 기존 지표와 상관관계가 높아 새 정보량이 적다"는 가설과 일치. **bb_squeeze만 후속 조사 가치 있음**(ATR%와의 중복 여부 확인이 다음 단계) — 아직 신호 점수·예상 등락률 어디에도 미반영, 데이터로 결정할 때까지 코드는 정보성 계산만 유지
