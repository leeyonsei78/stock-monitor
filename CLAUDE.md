...
```yaml
buy_conditions:
  min_signal_score: 0.40     # 매수 최소 점수 (2026-08-12: 0.60→0.40 — 0.60은 구조상 도달 불가)
...
2. `min_signal_score` 증가 (0.40 → 0.55)
...
- `+0.40` 이상 → 매수 (2026-08-12: 0.60→0.40 완화 — 기술점수 정규화 구조상 0.60 도달 거의 불가)
- `-0.50` 이하 → 매도
- `-0.70` 이하 → 강한 매도
