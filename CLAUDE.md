...
사용자 요청으로 PER/PBR과 함께 후보에 올랐으나 "검토만" 하기로 함 — 아직 코드 변경 없음.
- **왜 유망한가**: 방금 검증한 투자자 수급 아카이브(외국인/기관 당일 순매수 여부)가 익일 수익률에 예측력이 없다는 결과가 나왔는데(위 "신호 성과 추적" 섹션 2026-09-11 항목 참고), 이건 전부 **EOD(장마감 후 집계) 데이터**의 한계일 가능성이 있음 — KIS 호가 TR(매수/매도 잔량, 체결강도)은 그 순간의 실시간 매수·매도 압력이라 성격이 근본적으로 다름. EOD 수급이 안 통했다고 실시간 수급도 안 통한다고 볼 근거는 없음
- **비용**: 종목당 새 API 호출 추가(스캔 시간 증가), 새 신호 성분 설계·검증(합성 데이터 테스트 + 실전 드라이런 + 데이터 축적 후 상관관계 분석)까지 다른 지표들과 동일한 전체 사이클이 필요해 PER/PBR보다 개발 공수가 훨씬 큼
- **다음 단계**: 사용자가 착수를 요청하면 ① KIS 호가 조회 TR(`inquire-asking-price`류로 추정, 정확한 TR 코드·필드명은 이 세션에서 미검증) 스펙 확인 ② `KISApi`에 조회 함수 추가 ③ 매수/매도 총잔량 비율 또는 체결강도를 정보성 지표로 우선 추가(다른 지표와 동일 원칙: 점수 미반영, 표시+기록만) ④ 데이터 쌓이면 상관관계 검증 — 순서로 진행 예정
