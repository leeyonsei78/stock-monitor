...
## 신호 임계값이 사실상 사문화된 상태 (2026-08-26 관측, 미수정 — 판단 보류)
...
- **판단 보류**: 위 "외국인 3일+ 연속매도 단독 매도 조건" 항목과 같은 사안 — 금요일 주간 리포트(`weekly_accuracy_report.py`)에 적중률 데이터가 쌓이기 전까지 결론 내지 말 것 ([[feedback-defer-ambiguous-signal-judgment]]). 다만 "매도 신호 = 종합점수 기반"이라는 문서상 설명과 실제 동작이 다르다는 사실은 여기 기록해둠
