...
monitor:
  ...
  watchlist:                 # 항상 감시할 종목 — 섹터별 대장주로 구성 (2026-08-24 4→15종목 확장)
    ...
    - "017670"               # SK텔레콤 (통신, 2026-08-25 추가)
    - "010950"               # S-Oil (정유/에너지, 2026-08-25 추가)
    - "090430"               # 아모레퍼시픽 (화장품, 2026-08-25 추가)
    - "352820"               # 하이브 (엔터테인먼트, 2026-08-25 추가)
...
**빈 섹터 4종목 추가 (2026-08-25, 같은 세션 후속)**: 사용자가 "또 다른 종목 추천해달라"고 요청 — 워치리스트에 아직 없던 산업 섹터(통신/정유·에너지/화장품/엔터테인먼트) 대표주 4개를 제안했고 사용자가 전부 추가하기로 선택: SK텔레콤(`017670`, 통신), S-Oil(`010950`, 정유/에너지), 아모레퍼시픽(`090430`, 화장품), 하이브(`352820`, 엔터테인먼트). 4종목 전부 `get_current_price`로 실측 검증 완료(전부 `market_name="코스피"`, `watchlist_markets` 추가 불필요). 검증 중 KIS 토큰 단기 레이트리밋(위 참고)에 한 번 걸렸음 — 직전 검증 호출(금 ETF) 후 1분 이내 새 `KISApi(store=None)` 인스턴스로 재호출해 403 발생, 65초 대기 후 재시도로 해결. 스크리닝(가격/거래량/시총) 미적용은 다른 워치리스트 종목과 동일.
...
1. `alert_cooldown_sec` 증가 (**2026-08-26: 1800 → 14400으로 변경 완료**, 아래 참고)
2. `min_signal_score` 증가 (0.35 → 0.50)
3. `scan_top_n` 감소 (30 → 15)
4. `volume_min_ratio` 증가 (1.3 → 2.0)

### 쿨다운 30분이 실제로는 한 번도 발동 못 하던 문제 (2026-08-26 수정)
`alert_cooldown_sec: 1800`(30분)이 **실제 스캔 간격보다 짧아 억제 기능을 전혀 못 하고 있었음**. 크론은 10분 간격이지만 GitHub Actions 스케줄 누락률이 높아(위 트리거 신뢰성 참고) 실제 실행은 훨씬 드문드문함 — 8/26 실측에서 동일 종목 연속 알림 간격이 **최소 34분 / 중앙값 64분**으로, 30분 미만 간격이 **0건**이었음(연속쌍 49개 전수 확인). 즉 쿨다운 조건에 걸린 알림이 단 하나도 없었음.
- **결과**: 그날 Slack 알림 63건 중 **28건(44%)이 순수 중복** — SK텔레콤(`017670`) "외국인 5일 연속 매도" **10회**, KB금융(`105560`) 9회, LG화학(`051910`) 9회가 각각 동일 사유·거의 동일 점수로 재발송
- **`should_alert()` 로직 자체는 정상** (같은 signal_type이 쿨다운 내면 억제, 방향 바뀌면 즉시 알림) — 값만 실제 실행 주기와 안 맞았던 것
- **수정**: `14400`(4시간)으로 변경. 실측 중앙값 64분의 약 4배라 하루 2~3회로 제한됨
- VM 이전(cron 10~30분 간격이 안정적으로 지켜짐) 후에는 이 값을 다시 낮출 여지가 있음 — 그땐 실제 간격 분포를 다시 측정하고 조정할 것

### 위 4시간 수정이 실제로는 반영되지 않고 있던 문제 (2026-08-27 수정)
`config.yaml`의 `alert_cooldown_sec`을 14400으로 바꿨지만 **실제 쿨다운 판정에는 여전히 30분이 쓰이고 있었음** — `run_monitor_once.py`가 `SupabaseSignalStore(supabase_url, supabase_key)`를 `cooldown_sec` 인자 없이 생성해 그 클래스 자체의 기본값(1800초)이 그대로 쓰였고, `RealtimeMonitor._cooldown_sec`(config에서 읽은 14400)는 Slack 표시 문구와 in-memory 폴백(Supabase 미설정 시)에만 쓰일 뿐 실제 판정 경로인 `store.should_alert()`엔 전달되지 않았음 — 두 클래스가 쿨다운 값을 각자 따로 들고 있어 config 변경이 조용히 한쪽에만 반영되는 구조였음.
- **발견 경위**: 8/27 배포 후 실전 로그 대조로 발견. 13:07 KST 스캔에서 알림이 나간 종목들이 3분 뒤(13:10) 재스캔에선 억제됐지만(1800초 이내), 58분 뒤(14:05) 재스캔에선 동일 종목·동일 신호타입(예: NAVER 매도→매도)인데도 다시 알림이 나감 — 1800초는 넘고 14400초는 안 넘은 구간이라 "실제 쿨다운이 30분"이라는 정확한 신호
- **수정**: `RealtimeMonitor.__init__`에서 `store`가 있으면 `store._cooldown_sec = self._cooldown_sec`로 강제 동기화 — 호출부가 store를 어떤 값으로 생성했든 config.yaml의 `alert_cooldown_sec`을 유일한 기준으로 삼음
- 이 버그로 인해 어제 밤 "63건 중 44% 중복" 문제를 잡으려던 수정이 하루 종일 무효 상태였고, 오늘(8/27) 낮에도 현대차·SK텔레콤·KODEX골드·삼성전자우·아모레퍼시픽·실리콘투·에이팩트·SK아이이테크놀로지 등이 30분 간격으로 계속 재알림된 것으로 확인됨

## 신호 임계값이 사실상 사문화된 상태 (2026-08-26 관측 → 2026-08-28 데이터 기반 조치)
8/26 매도 신호 46건을 전수 분석한 결과 **`sell_conditions.min_signal_score: -0.50`을 실제로 충족한 건이 0건**이었음. 전부 단독 조건 오버라이드로 발동:
| 발동 사유 | 건수 |
|---|---|
| 외국인/기관 연속매도 단독 | 29 |
| RSI 과매수 단독 | 10 |
| 미집계+급락 오버라이드 | 7 |
| **종합점수 -0.50 이하 (설정된 기준)** | **0** |

매도 신호의 종합점수 범위는 **-0.176 ~ +0.112**로 임계값 -0.50 근처에도 못 감. 즉 매도 판정은 종합점수가 아니라 사실상 단독 조건들이 전담하고 있음.
- 이 조건들에 붙어 있는 `score < 0` 가드가 -0.021 같은 값도 통과시켜 실질 필터 역할을 못 함
- **2026-08-28 금요일 주간 리포트로 데이터 축적 완료, 조치 실행**: 누적 265건(그중 매도 168건, 신뢰할 만한 표본) 기준 **매도 적중률 86/168 = 51.2%(거의 랜덤 수준)**, 매도 발동 시점 **평균 종합점수가 +0.115~+0.156으로 오히려 양수** — "매도가 사실상 종합점수와 무관하게 발동되고, 그 결과 예측력이 거의 없다"는 위 관측을 데이터로 확정. 판단 보류를 끝내고 조치:
  - `sell_conditions.standalone_score_max`(신규, 기본 -0.15) 추가 — RSI 과매수/외국인 연속매도 "단독조건" 매도의 `score < 0` 가드를 `score < standalone_score_max`로 강화. -0.021 같은 사실상 0에 가까운 값을 걸러내되, -0.50(순수 종합점수 매도선)보다는 훨씬 완화된 절반 수준이라 이 단독조건들의 존재 의미(종합점수 미달이어도 확실한 하락 신호 포착)는 유지
  - `stale_data_override`(당일 급락 시 매도)는 설계상 종합점수 무시가 의도된 것이라 이번 조치와 무관하게 그대로 유지 — 이건 여전히 별개 사안으로 판단 보류 중(아래 참고)
  - `weekly_accuracy_report.py`에 **매도 사유별(종합점수기반/RSI과매수단독/외국인연속매도단독/당일급락오버라이드) 세부 적중률** 추가 — 기존엔 "오전 연속매수/매도 신호 N건"으로 빈도만 셌지 적중률은 안 보여줘서, 이번 가드 강화가 실제로 도움됐는지 판단할 근거가 없었음. 카테고리는 배타적이지 않음(reason이 여러 사유를 " / "로 동시에 담을 수 있어 한 신호가 여러 카테고리에 겹쳐 들어갈 수 있음), 카테고리별 최소 5건 미만이면 생략
  - **다음 주 리포트부터 이 가드 강화 전후 적중률 비교로 효과 검증할 것** — standalone_score_max=-0.15는 데이터 기반 최적값이 아니라 "0보다는 확실히 엄격한, 하지만 -0.50보다는 완화된" 중간값으로 잡은 1차 조정치. 다음 주 리포트의 카테고리별 적중률을 보고 추가 조정 여부 판단
  - **다음 검토 예정일: 2026-09-04(금) 주간 리포트(18:30 KST) 확인 후** — Claude 세션에 리마인더 예약해둠(`weekly_accuracy_report.py`의 `_sell_reason_breakdown()` 카테고리별 적중률로 -0.15 가드 효과 판단, 필요시 재조정). 이번 주(8/28) 리포트는 가드 강화 전 데이터라 비교 대상 아님 — 9/4 리포트가 강화 후 첫 유의미한 데이터
...
- `-0.50` 이하 → 매도
- `-0.70` 이하 → 강한 매도
- RSI 과매수(≥70) 매도 조건: **종합 점수 < 0 일 때만 발동** (수급 양호 급등주 오발화 방지, 2026-08-11) — **2026-08-28: 이 임계값 자체가 `score < standalone_score_max(-0.15)`로 강화됨, 아래 2026-08-28 하위 항목 참고 (이 줄은 원래 도입 배경 기록용으로 남겨둠, 현재 동작 아님)**
- **외국인 3일+ 연속매도 단독 매도 조건도 종합 점수 < 0 일 때만 발동하도록 수정 (2026-08-21, 잠정, ⚠️ 2026-08-28에 아래 하위 항목으로 -0.15로 강화됨 — 이 줄도 원래 도입 배경 기록용)**: RSI 조건에만 있던 `score<0` 가드가 이 조건엔 2026-08-02 최초 커밋 이후 계속 빠져 있었음 — 히스토리 버그 수정(위 참고)으로 `foreign_streak`가 실제로 작동하기 시작하자 우리기술투자(041190)가 종합점수 +0.46~0.47(매수권, 오히려 상승 중)인데도 16:00~17:53 KST 5회 연속 스캔에서 매도로 분류되는 사례 실측 → RSI 조건과 동일하게 가드 추가(`51c2342`)
  - **2026-08-28 데이터 기반 재판단 완료**: 금요일 주간 리포트 누적 데이터(매도 168건, 적중률 51.2%로 거의 랜덤)로 "이 단독조건이 종합점수와 무관하게 너무 쉽게 발동된다"는 우려가 확인됨 — 다만 "외국인 연속매도 자체가 무의미하다"는 결론까지는 아직 못 내림(카테고리별 세부 적중률이 이번 주 리포트엔 처음 추가돼 데이터가 아직 1주치뿐). 절충안으로 `score<0` 가드를 `score < standalone_score_max(-0.15)`로 강화(아래 "신호 임계값이 사실상 사문화된 상태" 섹션 참고) — "당일 반등과 무관한 독립적 경고"라는 이 조건의 존재 의미는 유지하되 사실상 0에 가까운 애매한 경우만 걸러냄
  - **계속 관찰**: `weekly_accuracy_report.py`의 매도 사유별 세부 적중률(`외국인연속매도단독` 카테고리)로 이 가드 강화 전후 적중률 비교를 다음 주부터 이어갈 것
...
- **장중 비대칭 버그 수정 (2026-08-28)**: 업종 ETF 상대강도(`_calc_sector_rs`, 위 참고)에서 먼저 발견한 것과 동일한 버그가 여기에도 2026-08-21 도입 당시부터 있었음 — `technical_indicators.get_technical_score()`의 `stock_5d_return`은 `_analyze_stock`이 오늘 실시간가를 주입한 종목 OHLCV를 쓰는데, `index_5d_return`(`self._index_ohlcv`)엔 이 주입이 없어서 장중엔 "종목=오늘 실시간가 vs 지수=전일 종가"로 비교 기준일이 어긋났음. 사용자 질의로 재확인해 발견 — **이쪽은 `relative_strength` 가중치 15%로 종합점수에 직접 반영돼(섹터RS는 0%, 정보성 전용) 실질 영향이 더 큼**: 시장 전체가 장중 크게 움직인 날(위 8/26 "코스피 +5.21%/코스닥 +0.24%" 사례처럼)엔 종목의 당일 상승분만 상대강도에 반영되고 지수의 당일 상승분은 반영이 안 돼, 실제로는 "시장이 다 같이 오른 것"인데 "이 종목이 시장 대비 유독 강하다"로 왜곡됐을 가능성
  - 수정: `KISApi.get_index_current(code)` 추가 — `get_vkospi()`와 동일 TR(`FHPUP02100000`)·필드(`bstp_nmix_prpr`/`bstp_nmix_prdy_ctrt`, VKOSPI에서 이미 검증됨)를 코드만 바꿔 재사용(코스피="0001", 코스닥="1001"). `_scan_once()`에서 지수 OHLCV 조회 후 `_inject_today_row()`로 동일하게 오늘 값 주입(지수는 거래량 개념이 없어 게이트 통과용 더미 volume=1만 채움, 계산은 close만 사용해 무해)
  - **"0001"/"1001" 코드로 이 TR을 호출하는 것 — 2026-08-31 드라이런으로 정황상 확인됨(단, 100% 확증은 아님)**: KS11(0001) 반환값 6699.83이 같은 로그의 코스피200 선물(1053.35, 8/24 실측치 1051.65와도 스케일 일치)과 비교해 자연스러운 코스피 레벨로 보임 — idxcode.mst 매핑표(코스피=0001/코스닥=1001)가 맞다는 간접 증거. 에러 없이 상대강도 계산까지 정상 완료(스캔 47/47 처리, 오류 0개). 다만 이건 여전히 정황 증거이지 KIS 공식 문서로 확증된 건 아니라서(아래 방어 로직 참고 — 절대 레벨 하나만 보고는 코드가 맞는지 확실히 못 가려낸 사례가 바로 이번 것) 계속 관찰할 것
  - **코드 매핑이 틀렸을 경우의 방어 (2026-08-28 추가, 2026-08-31 절대범위→상대범위로 수정)**: 처음엔 `get_index_current()` 반환값을 절대적인 정상 범위(코스피 1000~6000, 코스닥 300~1500)와 대조하는 방식으로 방어했는데, **배포 전 실제 workflow_dispatch 드라이런(2026-08-31)에서 바로 오탐이 발생**: `KS11(0001) 지수현재가 6699.83가 정상범위(1000~6000) 벗어남` 경고가 떴음 — 그런데 이 값은 같은 로그의 코스피200 선물(1053.35, 위 2026-08-24 실측치 1051.65와도 스케일이 일치)과 비교하면 오히려 정상적인 코스피 레벨로 보임(코스피/코스피200 비율 상 자연스러운 크기). 즉 "0001" 코드 매핑 자체는 맞는데, 방어 로직에 박아둔 절대 레벨 가정(2026-08-24 시점 기준)이 시장이 계속 오르면서 낡아 오탐을 낸 것 — 지수 레벨은 시간이 지나며 계속 바뀌므로 절대범위 방식 자체가 근본적으로 계속 갱신해야 하는 취약한 접근이었음
    - **수정**: 절대범위 대신 같은 루프에서 이미 조회한 전일 종가(`ohlcv[-1]["close"]`) 대비 등락폭이 상식적 범위(±15%)인지로 판단 — 지수 레벨이 얼마든 상관없이 항상 유효한 자기 일관성 체크로 교체. 정상/코드매핑오류 두 케이스 모두 유닛 테스트로 재확인(정상 시나리오는 실제 드라이런에서 관측된 6699.83/전일 6650 가정 등 반영)
    - 이 방식도 완벽한 보장은 아님(코드가 틀렸는데 마침 전일 대비 등락폭도 그럴듯한 경우는 여전히 못 잡음) — 그래도 절대범위처럼 시간이 지나며 스스로 낡는 문제는 없음
  - **코드 리뷰로 발견한 추가 결함 3건 수정 (2026-08-31)**: 이 기능 전체에 정식 code-review를 아직 안 돌려서 놓치고 있던 문제들
    - **`get_daily_ohlcv`와 `get_index_current`가 같은 try 블록에 있어 한쪽만 실패해도 둘 다 버려짐**: `get_index_current()`(신규 추가)만 실패해도(장중 KIS 지수 조회 API 일시 장애 등) 이미 성공한 30일치 `get_daily_ohlcv` 결과까지 통째로 버려지고 `self._index_ohlcv[key] = None`이 되어 그 시장 상대강도 전체가 꺼짐 — 이 기능 추가 전엔 `get_daily_ohlcv` 실패만이 이 상태를 만드는 유일한 원인이었는데, 신규 API 호출 하나가 실패 원인을 조용히 넓힌 것. `get_daily_ohlcv` 성공 후 `get_index_current`/주입 로직을 별도 try로 분리해 후자만 실패해도 전일 종가 기준 ohlcv는 그대로 씀(기능 추가 이전과 동일하게 안전 후퇴)
    - **전일 종가 없음(`prev_close<=0`)과 "코드 매핑 오류 의심"이 같은 경고 문구를 쓰고 있었음**: OHLCV 조회 자체는 성공했지만 빈 리스트가 온 경우(당일 신규상장 등 극히 드문 케이스)에도 "코드 매핑 오류 의심" 경고가 떠서, 실제로는 데이터가 없는 것뿐인데 코드 매핑 문제로 오인하게 만들었음 — 이 방어 로직의 목적이 정확히 "코드 매핑 확인"이라 원인 구분이 중요해 별도 문구("전일 종가 데이터 없음")로 분리
    - **`KISApi.get_index_current()` docstring이 이미 폐기된 절대범위 가이드를 그대로 담고 있었음**: 위에서 절대범위→상대범위로 고쳐놓고 이 함수 자체의 docstring은 안 고쳐서 "배포 전 코스피 2000~4000대인지 확인할 것"이라는, 이미 틀렸다고 확인된 조언이 남아있었음 — 나중에 이 함수를 재사용하는 코드가 이 낡은 가이드를 그대로 믿을 위험. 현재 상태(정황상 확인됨, 상대범위 검증 방식 안내)로 갱신
    - 4가지 케이스(정상/get_index_current 실패/get_daily_ohlcv 실패/빈 ohlcv) 유닛 테스트로 확인
...
7. **예상 등락률** (2026-08-21 추가, 2026-08-24 명칭 변경): ATR(변동성) × 신호강도 기반 경험적 추정치 + 산출 근거 텍스트 (`TradeSignal.expected_return_pct/expected_return_basis`) — 통계 검증된 예측 아님, 실제 신호-결과 데이터(`evaluate_signals.py`)가 쌓이면 회귀모델로 교체 예정
   ...
   - **크기(신뢰도) 산출 버그 수정 (2026-08-28)**: 8/28 주간 리포트 실측(예상등락률 방향적중 92/206=44.7%로 랜덤보다 낮음, 가상매매 완결거래 기준 1/8=12.5%)을 계기로 재검토 — 위 방향 버그는 부호만 고쳤을 뿐, 크기(confidence)는 여전히 그 반대 방향을 가리키는 `|score|`를 그대로 썼던 게 남아있었음. 8/24 예시로 보면 SELL인데 confidence를 score=+0.288(강세)의 크기(38%)로 산출해 "반대 방향에 대한 확신이 클수록 이 방향의 확신도 크다"는 모순이 그대로 있었던 것 — 방향 버그가 부호는 고쳤지만 크기는 안 고친 반쪽 수정이었음. `score` 부호가 `signal_type`이 정한 방향과 일치할 때만 `|score|`를 confidence로 쓰고, 어긋나면(주로 `stale_data_override`로 방향이 강제된 경우) 근거 없는 확신도를 만들지 않고 `multiplier_min`(0.5배)만 적용하는 보수적 추정으로 변경. 위 44.7%/12.5%라는 수치 자체는 이 크기 버그 때문이라기보다 매도/관심 신호 자체의 낮은 적중률(위 "신호 임계값이 사실상 사문화된 상태" 참고)이 그대로 반영된 것이라 이 수정만으로 크게 개선되진 않을 것으로 예상 — 다만 방향이 반대인 상황에서 근거 없이 부풀려진 예상 등락률 폭 자체는 이 수정으로 제거됨. 데이터가 더 쌓이면 이 변경의 효과도 MAE로 확인할 것
...
## 스캔 요약 메시지 (2026-08-26 추가)
스캔이 끝날 때 그 스캔에서 나온 **매수/매도/관심 신호를 종목별 한 줄로 묶은 요약 메시지**를 별도 전송 (`realtime_monitor._send_scan_summary`).
- **도입 이유**: 종목별 상세 알림은 정보량이 많아 유용하지만, 스캔 종목이 43개로 늘면서(위 스캔 범위 수정) 종목이 많을 때 개별 메시지 사이에서 놓치기 쉽다는 사용자 지적
- **쿨다운과 무관하게 매 스캔 전송** — 상세 알림은 `alert_cooldown_sec`(4시간)로 중복을 줄이고, 이 요약이 "그 순간의 전체 현황"을 보장하는 역할 분담. 쿨다운 상향(위 참고)과 짝을 이루는 기능이라 둘을 같이 봐야 함
- **형식**: `• 종목명 (티커) · 현재가 · 당일등락률 · 점수` — 그룹은 강한매수→매수→관심→매도→강한매도 순, 그룹 내부는 매수 계열이 점수 높은 순 / 매도 계열이 점수 낮은 순(각 그룹에서 가장 강한 신호가 위로)
- HOLD는 종목을 나열하지 않고 건수만 표시(요약 목적이 흐려지므로), 비-HOLD 신호가 하나도 없으면 메시지 자체를 생략(매 스캔 "특이사항 없음" 스팸 방지)
- 그룹당 최대 15종목 표시 후 `… 외 N종목`(`SUMMARY_MAX_PER_GROUP`) — Slack 4000자 제한 방어. 실측 렌더링은 신호 12건에 640자
- 전송 실패해도 스캔 자체는 계속 진행(로그만 남김)
- `config.yaml monitor.scan_summary: false`로 끌 수 있음
- **"보유" 건수 계산 버그 수정 (2026-08-27)**: 요약 헤더의 `보유 N건`이 `processed - signal_total`로 계산돼 있었는데, `processed`는 그 스캔에서 스킵(데이터 30일 미만 등)·오류(예외) 종목까지 포함한 "시도한 종목 수"라 실제 분석 성공 종목 수(`len(scan_signals)`)보다 큼 — 스킵/오류가 하나라도 있으면 그 개수만큼 보유 건수가 부풀려지는 버그(코드 리뷰로 발견, 실행 후 발생하는 유형이라 로그만으로는 안 드러남). `len(scan_signals) - signal_total`로 수정, 분모 표기도 "종목 스캔"→"종목 분석"으로 정정(실제 분석 완료 수를 뜻하도록)
...
- **토큰 발급 단기 레이트리밋 (2026-08-24 실측)**: "1일 1회" 제한과 별개로, 직전 발급 후 짧은 시간(대략 1분 이내) 안에 새로 발급 시도하면 `403 Forbidden`(`/oauth2/tokenP`) 발생. Supabase 캐시를 안 쓰는 `KISApi(store=None)`(로컬 검증 스크립트 등)를 짧은 간격으로 여러 번 새로 띄우면 재현됨
- **토큰 캐시가 이제 기본값 (2026-08-26 수정)**: 위 문제의 근본 원인은 `KISApi.__init__(store=None)`이라 **캐시 미사용이 기본 동작**이었던 것 — 명시적으로 `store=`를 넘기는 곳(`realtime_monitor`/`evaluate_signals`/`archive_investor_data`)만 캐시를 썼고, `src/main.py`와 `api/routers/trades.py`는 둘 다 `KISApi()`로 생성해 **실행할 때마다 토큰을 새로 발급**하고 있었음(1일 1회 한도를 조용히 갉아먹는 상태). 로컬 검증 스크립트도 같은 함정에 반복해서 걸림
  - **수정**: `store`를 안 넘기면 `SUPABASE_URL`/`SUPABASE_KEY`가 환경에 있을 때 토큰 캐시용 store를 **자동으로 붙임**. 검증: 연속 두 `KISApi()` 인스턴스가 둘 다 `Supabase 캐시 토큰 재사용` 로그를 남기고 신규 발급 0회
  - 캐시를 명시적으로 끄려면 `KISApi(use_token_cache=False)`
  - Supabase 자격증명이 없거나 연결 실패 시엔 경고 로그만 남기고 기존 동작(매번 발급)으로 degrade — 예외를 올리지 않음
  - `_default_token_store()`의 supabase import는 함수 내부 지연 로딩 — 최상단 import로 두면 KIS만 쓰는 경량 스크립트까지 supabase 패키지를 강제로 끌고 오게 됨
  - **로컬에서 임시 검증 스크립트를 쓸 때도 이제 그냥 `KISApi()`를 쓰면 됨** (별도 store 배선 불필요)
...
- **주간 정확도 추이 리포트 (2026-08-21 추가)**: `weekly_accuracy_report.py` — 1일차 평가 완료된 신호 전체를 월~일 주 단위로 묶어 매수/매도 적중률·예상등락률 MAE의 주차별 추이 + 누적 요약을 Slack으로 전송
  ...
  - **매도 사유별 세부 적중률 추가 (2026-08-28)**: 위 "연속" 빈도 카운트는 빈도만 셀 뿐 적중률은 안 보여줘서 원래 도입 목적(외국인 연속매도 가드가 실제로 도움되는지 판단)에 못 미쳤음 — `_sell_reason_breakdown()` 추가로 종합점수기반/RSI과매수단독/외국인연속매도단독/당일급락오버라이드 4개 카테고리별 적중률·평균수익률·평균종합점수를 누적 기준으로 표시(카테고리별 최소 5건, 배타적 분류 아님). 도입 계기와 이 리포트의 첫 실측 결과(누적 매도 168건 적중률 51.2%)는 아래 "신호 임계값이 사실상 사문화된 상태" 섹션 참고 — 그 결과로 `sell_conditions.standalone_score_max` 가드 강화도 같이 진행함
...
## 스캔 시장 범위 (2026-08-20 → 2026-08-26 전면 수정)
- `FHPST01710000` TR은 `FID_COND_MRKT_DIV_CODE="J"`만 지원 ("Q" 전달 시 API 오류)
- `FID_BLNG_CLS_CODE` "1"/"2"는 시장 구분이 아닌 종목등급 코드 → 이 파라미터로는 KOSPI/KOSDAQ 분리 불가 (2026-08-20 진단)
  - "1" 결과: 스팩·ETN 등 비정형 종목, "2" 결과: KOSPI ETF + 일부 주식 (혼재)

### **`FID_INPUT_ISCD`로는 시장 분리가 된다 (2026-08-26 실측 발견 — 위 "분리 불가" 결론을 부분 정정)**
2026-08-20 결론은 `FID_BLNG_CLS_CODE`에 대한 것이었고, **다른 파라미터인 `FID_INPUT_ISCD`는 시장 분리가 정상 동작**한다. 실측 확인값:
| `FID_INPUT_ISCD` | 결과 |
|---|---|
| `0000` (기존 사용값) | 전체 — 상위권을 KODEX 200선물인버스·KODEX 인버스·TIGER 200선물인버스 등이 점유 |
| `0001` | 코스피 (대우건설·삼성전자·금호건설·팬오션…) |
| `1001` | 코스닥 (신라젠·모아데이타·희림·대한광통신…) |
| `2001` | 코스피200 |

**발견 경위 (2026-08-26 오늘 스캔 결과 리뷰)**: 로그 전체에서 `거래량 상위 조회 완료: 6~7개 추가 (목표 30개)`(2회는 0개)가 반복되는 걸 발견 — `scan_top_n: 30`이 한 번도 채워진 적이 없었음. 원인 2가지:
1. **`get_top_volume_stocks(limit=100)`의 `limit=100`은 죽은 값** — 이 TR은 요청 파라미터와 무관하게 **항상 30행만** 반환하고 페이징도 없음(`tr_cont` 헤더 빈 문자열, `ctx_area` 키 없음 — 실측 확인). `FID_DIV_CLS_CODE` 0/1/2를 바꿔도 항상 30행
2. **그 30칸의 대부분이 스크리닝에서 탈락** — 실측 내역: ETF 키워드 10개 / ETN 등 비6자리 코드 6개 / 가격범위 탈락 7개 → **통과 7개**
- **수정**: `0001`(코스피)·`1001`(코스닥)로 나눠 2회 조회 후 합침 — 각 30행이 전부 실제 종목이라 **통과 26개**로 늘어남(실측: 코스피 17 + 코스닥 9). 배포 전 드라이런에서 코스피 15 + 코스닥 8 = 23개 추가, 총 스캔 대상 43개(워치리스트 20 + 스캔 23) 확인
- **부수 효과 — 상대강도 벤치마크 정확화**: 시장 구분이 가능해져 코스닥 종목은 `KQ11`, 코스피는 `KS11`로 비교(기존엔 전 종목 `KS11` 근사치). 오늘처럼 코스피 +5.21% / 코스닥 +0.24%로 벌어진 날(괴리 4.97%p)엔 5일 수익률 0~10% 구간의 코스닥 종목 상대강도 부호가 통째로 뒤집힘(예: -0.20 → +0.20). 상대강도 가중치가 15%라 종합점수 약 0.06 차이 — 매수선 0.30 기준에서 무시 못 할 크기
  - `_index_ohlcv`가 단일 리스트 → `{"KS11": [...], "KQ11": [...]}` dict로 변경, `_analyze_stock(index_key=)`로 종목별 전달. 조회 실패 시 해당 시장만 `None` → 상대강도 중립(0.0) degrade (기존 동작과 동일)
  - 워치리스트는 종목 고정이라 `WATCHLIST_KOSDAQ_TICKERS`(`041190`/`277810`) 상수로 구분
- **⚠️ 주의: 여기서 얻은 시장 구분은 벤치마크 선택에만 쓰고 API 호출용 `market` 코드로는 절대 쓰지 말 것** — `get_current_price`(FHKST01010100)는 `"Q"`를 아예 안 받고, 코스닥 종목도 `"J"`로 정상 조회됨(2026-08-24 확인, 2026-08-26에 코스닥 3종목 `215600`/`288980`/`041190`으로 재확인 — 전부 `market="J"`로 조회되고 `market_name`이 "코스닥"으로 정상 반환). 모든 스캔 종목은 계속 `market="J"`로 처리
- 시장별 조회는 독립 try/except — 한쪽이 실패해도 다른 쪽은 계속 진행 (2026-08-14 코스피/코스닥 에러 분리 원칙과 동일)

### 8/26 배포 후 첫 실전 검증 (2026-08-27 09:09 KST 수동 실행)
장 시작 직후 사용자가 직접 `workflow_dispatch`로 실행해 8/26 밤 수정사항(스캔 확대·토큰 캐시·쿨다운)이 실제로 반영됐는지 확인:
- **스캔 후보 24개 추가**(목표 30개) — 수정 전 6~7개 대비 대폭 개선, 배포 전 드라이런 추정치(23~26개)와 부합. 총 44종목(워치리스트 20 + 스캔 24) 처리, 스킵 0·오류 0, 소요 196초(`MAX_SCAN_SEC=720`에 여유)
- **KIS 토큰 캐시 정상 동작 확인**: `KIS 토큰 Supabase 조회 성공 (만료: ...22:19:34)` → 이미 만료된 상태라 재발급 → `Supabase 저장 성공`까지 캐시 조회→만료 판단→재발급→저장 전 과정이 로그로 확인됨
...
### 1순위 ② 공매도 비중 (`src/api/krx_data.py`, 신규 의존성 `pykrx`) — ⚠️ 2026-08-28 드라이런으로 "API 키 불필요" 전제가 틀렸음을 확인, KRX_ID/KRX_PW 등록 후 정상 동작 확인 완료
...
- `requirements.txt`에 `pykrx>=1.0.51` 추가 (실제 설치된 버전 1.2.8)

**⚠️ 실측(2026-08-28 GitHub Actions 드라이런 3회)으로 처음 알게 된 사실 — "API 키 불필요"는 틀렸음**: 이 기능 최초 작성 시 pykrx를 "API 키 불필요"로 문서화했으나, 실제 드라이런에서 `get_shorting_volume_top50()`이 **오늘 날짜뿐 아니라 확실한 과거 실거래일(2024-01-02)에도 예외 없이 계속 빈 응답만 반환**하는 걸 발견. 매 로그 끝에 `KRX 로그인 실패: KRX_ID 또는 KRX_PW 환경 변수가 설정되지 않았습니다`가 함께 찍혀 있었고, pykrx 1.2.8 패키지 메타데이터에 "KRX 로그인이 필요한 API를 사용하려면 KRX_ID/KRX_PW 환경변수가 **필수**"라고 명시돼 있음을 로컬에서 pykrx 소스를 직접 열어 확인.
- **DART_API_KEY와 성격이 다름**: DART_API_KEY는 무료 발급형 API 키(스코프 제한적)인 반면, `KRX_ID`/`KRX_PW`는 **사용자의 실제 KRX 회원 로그인 아이디/비밀번호 그 자체**임 — GitHub Actions에 개인 로그인 자격증명을 올려 무인 스캔마다 자동 로그인시키는 건 보안 성격이 다른 결정이라 이 세션에서 임의로 추가하지 않았고, 사용자에게 트레이드오프를 안내한 뒤 사용자가 직접 data.krx.co.kr에 회원가입하고 GitHub Secrets에 등록하기로 결정함
- **부수 성과**: 이 진단 과정에서 컬럼명도 실제 pykrx 소스(docstring)로 정확히 확인함(`공매도비중`) — 최초 작성 시 추측이었던 후보 목록(`비중`/`숏비율`/`공매도 비중(%)`)엔 정답이 없었음
- **2026-08-28 등록 후 재검증 완료**: 사용자가 `KRX_ID`/`KRX_PW`를 GitHub Secrets에 등록 → `stock_monitor.yml`에 전달 배선 추가(`43a72ee`) → 드라이런으로 `KRX 로그인 완료` + `공매도 비중 데이터 101종목 확보 (기준일 20260827)` 확인. 실제 인증·데이터 수집까지 전부 정상 동작. `KRX_ID`/`KRX_PW` 미설정 시엔 `get_short_interest_ratios()`가 pykrx를 아예 호출하지 않고 즉시 빈 dict 반환하도록 방어돼 있어(다른 배포 환경에서 재사용 시에도) 안전
- **⚠️ 실전 운영 중 관측(2026-08-28, 실제 스케줄 스캔 로그, 사용자 제보)**: `=== 스캔 완료 ===` 로그 이후, 스크립트 종료 시점에 `Error occurred in get_shorting_volume_top50: "None of [Index(['RANK', 'ISU_CD', 'CVSRTSELL_TRDVAL', ...])] are in the [columns]"`가 2회(코스피·코스닥 추정) 출력됨. 이 프로젝트 코드가 호출한 게 아님 — `get_short_interest_ratios()`는 스캔 시작부에서 이미 해당 스캔에 필요한 데이터를 다 받아온 뒤라(로그상 101종목 확보가 스캔 시작 직후 찍혀 있고, 이 함수는 성공하면 그대로 반환해 재호출 안 함) 이 에러 시점엔 이미 함수 실행이 끝난 상태. pykrx 라이브러리 자체가 세션 정리(KRXSession 소멸자 등 추정) 과정에서 내부적으로 한 번 더 호출하면서 발생하는 것으로 보이며, `print()`로 직접 출력돼 우리 쪽 logger 포맷과도 다름(원인 확인용 코드 스니펫: `src/api/krx_data.py`의 로그 포맷과 비교해 다른 곳에서 찍힌 게 명백). 스캔 자체는 이미 `success`로 완료된 뒤라 결과·알림엔 영향 없는 것으로 판단됨 — 다만 pykrx 내부 동작이라 이 프로젝트 코드로 직접 고칠 수 없고, 향후 pykrx 업데이트로 사라지거나 반대로 실제 문제로 발전할 수 있어 계속 지켜볼 것. 판단 보류(코드 수정 안 함)
...
- **2026-08-28 등록 후 재검증 완료**: 사용자가 `DART_API_KEY`를 발급받아 GitHub Secrets에 등록(`stock_monitor.yml`엔 이미 passthrough 배선돼 있었음) → 드라이런으로 `DART 오늘 공시 종목 127개 확인` 로그 확인, 페이지네이션 포함 정상 동작
...
- **장중 비대칭 버그 수정 (2026-08-28, 코드 리뷰로 발견)**: 종목 쪽 OHLCV는 `_analyze_stock`에서 오늘 실시간가를 마지막 행으로 주입하는데(FDR 당일 데이터 지연 보완, 위 참고), 최초 구현 시 업종 ETF 쪽(`_sector_ohlcv`)엔 이 주입이 빠져 있어서 장중엔 "종목=오늘 실시간가 vs ETF=전일 종가"로 비교 기준일이 어긋났음 — ETF 자체의 장중 변동분이 고스란히 상대강도 오차로 새어 들어가는 구조. 주입 로직을 `_inject_today_row()` 공통 헬퍼로 추출해 ETF 조회 시에도 동일 적용(`get_current_price(etf, market="J")` 추가 호출, 스캔당 매핑된 ETF 종류 수만큼만 — 현재 1개)해 수정. KS11/KQ11 대비 상대강도(`_index_ohlcv`)도 동일한 비대칭이 있을 것으로 의심됐는데(2026-08-21 도입 당시부터 있었을 코드라 이번 리뷰 범위 밖으로 남겨뒀었음), 사용자 재확인 요청으로 검증한 결과 실제로 동일한 버그였음 — 위 "지수 대비 상대강도" 섹션의 2026-08-28 항목 참고, 별도 수정 완료
  - **`get_daily_ohlcv`+`get_current_price` 같은 try 결합 문제도 동일하게 수정 (2026-08-31)**: KS11/KQ11 쪽에서 먼저 발견한 문제(위 "지수 대비 상대강도" 섹션 2026-08-31 항목 참고) — `get_current_price(etf, ...)`만 실패해도 이미 성공한 10일치 `get_daily_ohlcv` 결과까지 버려지던 것을 별도 try로 분리해 동일하게 수정
...
- [x] `workflow_dispatch` 드라이런 3회 실행 및 로그 확인 — 완료, 아래가 실측 결과:
  - **S&P500/USD-KRW**: 정상 동작 확인(`S&P500: 7731.0 (+0.72%, 기준 20260827)`, `USD/KRW: 1379.7 (-0.28%)`). 단 최초 버전은 USD/KRW 등락률이 `nan`으로 나오는 버그가 있었음(FDR 응답에 NaN Close 행이 섞여 `prev_close<=0` 체크가 못 걸러냄) — `dropna()` 후 계산하도록 수정, 재드라이런으로 정상값 확인(`9f6f60f`)
  - **공매도 비중**: ⚠️ 최초 드라이런 시점엔 비활성이었으나(pykrx가 KRX 회원 로그인을 요구하는 걸 뒤늦게 발견), **2026-08-28 사용자가 `KRX_ID`/`KRX_PW`를 발급받아 GitHub Secrets에 등록 + 워크플로우 전달 배선 추가(`43a72ee`) 후 재드라이런으로 정상 동작 확인**: 로그에 `KRX 로그인 완료` + `공매도 비중 데이터 101종목 확보 (기준일 20260827)` 확인 — 컬럼명(`공매도비중`) 파싱도 정상, 스키마 불일치 WARNING 없음. 이 스캔에서 알림이 뜬 종목 중 공매도 상위 101위 안에 든 종목이 없어 Slack 메시지에 `🩳 공매도 비중` 줄이 실제로 찍히는 것까지는 못 봤지만 데이터 수집 자체는 완전히 검증됨
  - **DART**: 최초 드라이런엔 `DART_API_KEY` 미설정 상태로 "공시 조회 건너뜀" 정상 확인, **2026-08-28 사용자가 발급받아 GitHub Secrets에 등록 후 재드라이런으로 정상 동작 확인**: 로그에 `DART 오늘 공시 종목 127개 확인` 확인 — 페이지네이션 포함 전 과정 정상 동작
  - **섹터 ETF(091160)**: 조회 실패 로그 없음 — 정상 동작 확인(005930/000660이 매 스캔 보유(HOLD)로만 나와 Slack 알림 텍스트로 "업종 ETF 대비" 줄이 실제로 뜨는 것까지는 못 봄, 계산 자체는 에러 없이 완료됨)
  - **min_trading_value 스크리닝**: 기본값 0(미적용)이라 스캔 결과 변화 없음 확인(거래량 상위 조회 결과 개수가 이전과 동일한 패턴)
- [x] `DART_API_KEY` 발급 후 GitHub Secrets(Actions) 등록 — 완료, 공시 조회 정상 동작 확인(위 참고)
- [x] `KRX_ID`/`KRX_PW` 등록 — 완료, 공매도 비중 기능 정상 동작 확인(위 참고)
