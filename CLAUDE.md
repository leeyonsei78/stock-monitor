...
- **결론**: ADX 필터/CMF/OBV 셋은 전부 기존 지표의 노이즈 수준(±0.03)을 못 벗어나 production 반영 근거 없음 — 셋 다 "오실레이터/거래량 계열의 파생 지표는 기존 지표와 상관관계가 높아 새 정보량이 적다"는 가설과 일치. **bb_squeeze만 ATR%와 별개의 정보량을 담고 있는 것으로 확인**(위 참고) — 아직 신호 점수·예상 등락률 어디에도 미반영, 실사용 여부는 데이터가 더 쌓인 뒤 사용자 판단으로 결정할 것
