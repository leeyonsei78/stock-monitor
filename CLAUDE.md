...
- **배포 후 첫 실전 드라이런 검증 (2026-09-02 08:16 KST, `workflow_dispatch`, run 33570232894)**:
  ...
  Supabase 마이그레이션 섹션 참고). 다만 이 스캔에서 발생한 유일한 알림(한화에어로스페이스
  012450, 매도)이 공시 8종목에 포함되는지는 로그에 종목별 매핑이 안 찍혀 확인 못 함 —
  `📋공시(호재)` 등 배지가 실제 Slack 메시지에 렌더링되는 모습 자체는 아직 육안 확인 전, 공시
  있는 종목에 알림이 뜨는 스캔에서 재확인 필요. 같은 스캔에서 VKOSPI(44.03)·코스피200선물
