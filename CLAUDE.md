...
- **당일 투자자 데이터 미집계 시 안전장치 (`stale_data_override`, 2026-08-24 추가)**: 위 `score<0` 가드들의 부작용이 실제 사례로 확인됨 — 8/24 삼성전자가 당일 -8.5% 급락 중인데도 당일 투자자 데이터가 아직 미집계라 전일(8/21, 급등일) 강세 수급 데이터가 그대로 쓰여 종합점수가 계속 양수(+0.24~0.29)로 나와 매도 조건이 전부 `score<0` 가드에 막혀 하루 종일 신호가 하나도 안 뜬 것을 실측으로 발견
  ...
  - **장전 시간대 "당일 급락/급등"이 실제로는 "전일 등락률" 오인이었던 버그 수정 (2026-09-03)**: `day_return_threshold` 판정에 쓰는 `day_return`은 `technical_indicators.py`가 OHLCV 마지막 두 행으로 계산하는데(`(현재가-전일종가)/전일종가`), 장전(08:00~09:00 KST)처럼 실시간 거래량이 아직 0이면 `_inject_today_row()`가 오늘 행 주입을 건너뛰어(`realtime_monitor.py:215` 조건) OHLCV 마지막 행이 여전히 "어제"에 머무름 — 이 경우 `day_return`은 "오늘 등락률"이 아니라 **어제(가장 최근 실제 거래일)의 등락률**을 담고 있는데, 코드는 이를 구분하지 못하고 그대로 "당일 급락/급등"으로 해석했음
    ...
    - **의도적으로 안 건드린 부분**: `day_return`은 볼린저 신호("당일 +3% 급등 시 돌파 지속 해석")와 매수 거래량 예외("당일+5% 급등 시 배율 미달 통과")에도 쓰이는데, 이 둘은 이번 수정 범위에서 제외 — 스코프를 이번에 확인된 stale override 오발동 건으로만 좁힘. 같은 시간대에 이 둘도 동일한 이유로 오염될 수 있다는 점은 남아있는 질문
