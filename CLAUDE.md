...
- **장중 비대칭 버그 수정 (2026-08-28)**: 업종 ETF 상대강도(`_calc_sector_rs`, 위 참고)에서 먼저 발견한 것과 동일한 버그가 여기에도 2026-08-21 도입 당시부터 있었음 — `technical_indicators.get_technical_score()`의 `stock_5d_return`은 `_analyze_stock`이 오늘 실시간가를 주입한 종목 OHLCV를 쓰는데, `index_5d_return`(`self._index_ohlcv`)엔 이 주입이 없어서 장중엔 "종목=오늘 실시간가 vs 지수=전일 종가"로 비교 기준일이 어긋났음. 사용자 질의로 재확인해 발견 — **이쪽은 `relative_strength` 가중치 15%로 종합점수에 직접 반영돼(섹터RS는 0%, 정보성 전용) 실질 영향이 더 큼**: 시장 전체가 장중 크게 움직인 날(위 8/26 "코스피 +5.21%/코스닥 +0.24%" 사례처럼)엔 종목의 당일 상승분만 상대강도에 반영되고 지수의 당일 상승분은 반영이 안 돼, 실제로는 "시장이 다 같이 오른 것"인데 "이 종목이 시장 대비 유독 강하다"로 왜곡됐을 가능성
  ...
  - ⚠️ **"0001"/"1001" 코드로 이 TR을 호출하는 것 자체는 아직 라이브 검증 전** — VKOSPI(코드 "0503")만 실측 확인됐고, idxcode.mst 체계상 0001/1001이 코스피/코스닥이라는 근거는 `get_vkospi()` 도입 시(2026-08-24) 마스터파일에서 확인한 코드 매핑표에 의존. **배포 전 workflow_dispatch 드라이런으로 로그의 지수값이 정상 범위(코스피 2000~4000대, 코스닥 500~1000대)인지, 상대강도 계산이 에러 없이 도는지 확인할 것**
  - **코드 매핑이 틀렸을 경우의 방어 (2026-08-28 추가)**: 위 라이브 미검증 상태로 배포되더라도 잘못된 값이 조용히 신호 점수에 섞여 들어가지 않도록, `_scan_once()`에서 `get_index_current()` 반환값을 대략적인 정상 범위(코스피 1000~6000, 코스닥 300~1500)와 대조 — 벗어나면 "코드 매핑 오류 의심" 경고 로그만 남기고 오늘 값 주입을 건너뜀(전일 종가 유지, 즉 이 기능 추가 이전과 동일한 동작으로 안전하게 후퇴). 정상 범위 안에 있다고 해서 코드 매핑이 반드시 맞다는 보장은 아니므로(예: 다른 지수가 우연히 비슷한 범위일 수 있음), 그래도 드라이런 로그 확인은 배포 전 필수
