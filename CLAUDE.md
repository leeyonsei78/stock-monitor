...
- **⚠️ 개발 환경 제약 — 이 세션 샌드박스는 KIS API가 차단돼 있어 `total_askp_rsqn`/`total_bidp_rsqn` 필드명을 라이브로 검증 못함**(다른 신규 지표 추가 때와 동일한 제약, PER/PBR보다 한 단계 더 불확실 — PER/PBR은 이미 매 스캔 호출 중인 TR의 필드 재사용이었지만 이건 아예 새로 추가하는 TR) — **배포 전 반드시 workflow_dispatch 드라이런으로 실제 값이 0이 아닌지 확인할 것.** 필드명이 틀리면 크래시 없이 조용히 0/None으로만 남고 WARNING 로그로 감지됨
- Supabase 마이그레이션 (수동 SQL 필요):
  ```sql
