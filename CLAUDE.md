...
- **장중 비대칭 버그 수정 (2026-08-28, 코드 리뷰로 발견)**: 종목 쪽 OHLCV는 `_analyze_stock`에서 오늘 실시간가를 마지막 행으로 주입하는데(FDR 당일 데이터 지연 보완, 위 참고), 최초 구현 시 업종 ETF 쪽(`_sector_ohlcv`)엔 이 주입이 빠져 있어서 장중엔 "종목=오늘 실시간가 vs ETF=전일 종가"로 비교 기준일이 어긋났음 — ETF 자체의 장중 변동분이 고스란히 상대강도 오차로 새어 들어가는 구조. 주입 로직을 `_inject_today_row()` 공통 헬퍼로 추출해 ETF 조회 시에도 동일 적용(`get_current_price(etf, market="J")` 추가 호출, 스캔당 매핑된 ETF 종류 수만큼만 — 현재 1개)해 수정. KS11/KQ11 대비 상대강도(`_index_ohlcv`)도 동일한 비대칭이 있는 것으로 보이나 이건 오늘 신규 추가가 아니라 2026-08-21 도입 당시부터 있었을 코드라 이번 리뷰 범위 밖 — 필요시 별도 확인할 것
