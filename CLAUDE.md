...
### 1순위 ② 공매도 비중 (`src/api/krx_data.py`, 신규 의존성 `pykrx`) — ⚠️ 2026-08-28 드라이런으로 "API 키 불필요" 전제가 틀렸음을 확인, 현재 사실상 비활성
...
- **DART_API_KEY와 성격이 다름**: DART_API_KEY는 무료 발급형 API 키(스코프 제한적)인 반면, `KRX_ID`/`KRX_PW`는 **사용자의 실제 KRX 회원 로그인 아이디/비밀번호 그 자체**임 — GitHub Actions에 개인 로그인 자격증명을 올려 무인 스캔마다 자동 로그인시키는 건 보안 성격이 다른 결정이라 이 세션에서 임의로 추가하지 않음. 실제로 쓸지는 사용자가 판단할 것(쓰기로 하면 `.env`/GitHub Secrets에 `KRX_ID`/`KRX_PW` 추가하면 됨 — 코드는 이미 두 값이 모두 있을 때만 시도하도록 구현돼 있음)
- **현재 상태**: `KRX_ID`/`KRX_PW` 미설정(기본) 상태에서는 `get_short_interest_ratios()`가 pykrx를 아예 호출하지 않고 즉시 빈 dict 반환 — 스캔에는 영향 없지만 공매도 비중 기능은 사실상 꺼져 있는 것과 같음
- **부수 성과**: 이 진단 과정에서 컬럼명도 실제 pykrx 소스(docstring)로 정확히 확인함(`공매도비중`) — 최초 작성 시 추측이었던 후보 목록(`비중`/`숏비율`/`공매도 비중(%)`)엔 정답이 없었음. `KRX_ID`/`KRX_PW`가 준비되면 컬럼명 문제 없이 바로 동작할 것으로 예상되나, 실제 인증 성공 케이스 자체는 검증 못 함(개인 계정 없이는 이 세션에서 확인 불가)
...
- [x] `workflow_dispatch` 드라이런 3회 실행 및 로그 확인 — 완료, 아래가 실측 결과:
  ...
  - **공매도 비중**: ⚠️ **현재 비활성 상태** — pykrx가 KRX 회원 로그인(`KRX_ID`/`KRX_PW`)을 요구하는 걸 뒤늦게 발견(위 "1순위 ② 공매도 비중" 섹션 참고). 코드는 안전하게 빈 dict로 degrade하지만 실제 데이터는 안 쌓임 — `KRX_ID`/`KRX_PW`를 쓸지는 보안 성격이 DART_API_KEY와 달라(개인 로그인 자격증명) 사용자 판단 필요
...
- [ ] `KRX_ID`/`KRX_PW` 등록 여부 결정 — 보안 트레이드오프 있음(위 참고), 안 하면 공매도 비중 기능은 계속 비활성
