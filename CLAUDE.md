...
- [x] `workflow_dispatch` 드라이런 3회 실행 및 로그 확인 — 완료, 아래가 실측 결과:
  ...
  - **DART**: `DART_API_KEY` 미설정 상태에서 "DART_API_KEY 미설정 — 공시 조회 건너뜀" 정상 확인. 실제 발급 후 등록하면 동작할 것으로 예상(발급 자체는 검증 못 함)
...
- [ ] `DART_API_KEY` 발급 후 `.env`(로컬)와 GitHub Secrets(Actions)에 등록 — 안 해도 나머지 기능은 정상 동작(공시만 계속 건너뜀), 아직 미등록
