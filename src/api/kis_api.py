...
class KISApi:
    ...
    def get_index_current(self, code: str) -> dict:
        ...
        "0001"/"1001" 코드 자체로 이 TR을 호출한 것은 2026-08-31 드라이런으로 정황상 확인됨
        (KS11 반환값이 같은 스캔의 코스피200 선물과 스케일이 일치, 에러 없이 스캔 완료) — 단
        여전히 정황 증거이지 KIS 공식 문서로 확증된 건 아님. 코드 매핑이 틀렸을 경우의 방어는
        realtime_monitor._scan_once()가 담당: 절대 지수 레벨로 검증하면 시장이 오르내리며
        계속 낡아 오탐이 남(2026-08-31 실측 — 코스피 2000~4000대라는 최초 가정이 실제 값
        6699.83에 바로 오탐을 냄), 그래서 전일 종가 대비 등락폭(±15%)으로 검증하는 방식으로
        교체됨 — 이 함수를 직접 호출하는 새 코드를 추가할 때도 절대 레벨 가정은 재사용하지 말 것.
        """
        data = self._get(
            "/uapi/domestic-stock/v1/quotations/inquire-index-price",
            "FHPUP02100000",
